...
export default function SavedScreen({ navigation }) {
  ...
  // We map the saved IDs to the actual hostel objects
  const favoriteHostels = hostels.filter(h => savedHostels.includes(h._id));

  const collections = [
    {
      _id: "fav1",
      name: "My Favorites",
      hostels: favoriteHostels
    }
  ];
  ...
  const renderHostel = (hostel) => (
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate("HostelDetail", { hostelId: hostel._id })
      }
    >
      <Image
        source={{
          uri: hostel.photos?.[0] || "https://via.placeholder.com/150",
        }}
      ...
      <View style={styles.hostelInfo}>
        ...
        <Text style={styles.hostelPrice}>
          ₹{hostel.rent?.single || hostel.rent?.sharing2}/mo
        </Text>
      </View>
    </TouchableOpacity>
  );
    ...
    <View style={styles.collectionCard}>
      ...
      {item.hostels && item.hostels.length > 0 ? (
        ...
        <Text style={styles.emptyText}>
          No hostels saved in this collection yet.
        </Text>
      )}
    </View>
  );
          ...
          ListEmptyComponent={
            <View style={styles.centerBox}>
              ...
              <Text style={styles.emptySub}>
                Tap the heart icon on any hostel to save it for later.
              </Text>
            </View>
          }
        />
